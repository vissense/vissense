 ...
 *
 * This function checks ´document.hidden´ (or vendor specific derivatives)
 * and will return `0` accordingly.
 *
 * @example
 *
 * var element = document.getElementById('myElement');
...
/**
 *
 * @typedef VisSenseConfig
 * @memberof VisSense
 * @type {Object}
 * @property {number} fullyvisible The percentage limit an element is
 * considered fully visible.
 * @property {number} hidden The percentage an element limit is considered
 * hidden.
 *
 ...
 * @class
 * @name VisSense
 * @throws {Error} Will throw an error if the first argument is not a DOM Element.
 *
 * @param {DOMElement} element A DOM element.
 * @param {VisSense.VisSenseConfig} [config={fullyvisible: 1, hidden: 0}] A configuration
 * object.
 *
...
/**
 * A number representation of an visibility state.
 * This is either 0, 1 or 2.
 *
 * @typedef VisStateCode
 * @memberof VisSense.VisState
 * @type {number}
 */

/**
 * A string representation of an visibility state.
 * This is either 'hidden', 'visible' or 'fullyvisible'.
 *
 * @typedef VisStateString
 * @memberof VisSense.VisState
 * @type {string}
 */
 ...
 *
 * @typedef  {Object} VisState
 * @memberof VisSense
 *
 * @property {VisSense.VisState.VisStateCode} code A number representation
 * of an visibility state.
 * @property {VisSense.VisState.VisStateString} state An string representation
 * of an visibility state.
...
VisSense.VisState = (function() {
  ...
  return {
     ...
     * @param {number} percentage A number between 0 and 1 which represents
     * the currently visible area of an element.
     * @param {VisState} [previous] The previous state of the element.
     *
     * @returns {VisState} A state object representing
     * the state "hidden".
     */
 ...
 * @memberof VisSense
 *
 * @param {VisSense.VisSense} visobj The VisSense instance to monitor.
 * @param {VisSense.VisMon#VisMonConfig} config A configuration object.
 *
 * @property {VisSense.VisSense} _visobj The given VisSense instance.
 * @property {VisSense.VisState|{}} _state The current state.
 * @property {function[]} _listeners An array of listeners.
...
function VisMon(visobj, config) {
  ...
  var strategies = isArray(_config.strategy) ?
    _config.strategy : [_config.strategy];
  ...
  for (var i = 0, n = me._events.length; i < n; i++) {
    if (_config[me._events[i]]) {
      me.on(me._events[i], _config[me._events[i]]);
    }
  }
}
 ...
 * @memberof VisSense.VisMon.Strategy
 *
 * @classdesc Despite the name of the class this method will actually initialize
 * the objects state once.
 *
 * *NOTE*: This class is improperly named and is likely to be renamed
 * or removed in the future.
 */
VisMon.Strategy.NoopStrategy = function () {
};

VisMon.Strategy.NoopStrategy.prototype = Object.create(
 ...
 * @method
 * @name start
 * @memberof VisSense.VisMon.Strategy.NoopStrategy#
 *
 * @param {VisSense.VisMon} monitor
 *
 * @description Initializes the state of the monitor once.
 */
...
VisMon.Strategy.EventStrategy.prototype.start = function (monitor) {
  ...
  if (!me._started) {
    ...
    if (VisibilityApi) {
      addEventListener(VisibilityApi[1], me._update);
    }
...
VisMon.Strategy.EventStrategy.prototype.stop = function () {
  ...
  if (VisibilityApi) {
    removeEventListener(VisibilityApi[1], me._update);
  }
  me._started = false;
