 ...
 * hidden.
 * @property {VisSense~GetVisiblePercentageCallback} [getVisiblePercentage=VisSense.Utils.percentage]
...
/**
 * @callback VisSense~GetVisiblePercentageCallback
 *
...
function VisMon(visobj, config) {
  ...
  this._pubsub.on('update', function () {
    if (me._state.code !== me._state.previous.code) {
      me._pubsub.publish('visibilitychange', [me]);
    }
  });
  ...
  this._pubsub.on('update', function () {
    ...
    if (newValue !== oldValue) {
      me._pubsub.publish('percentagechange', [newValue, oldValue, me]);
    }
  });

  this._pubsub.on('visibilitychange', function () {
    if (me._state.fullyvisible) {
      me._pubsub.publish('fullyvisible', [me]);
    }
  });

  this._pubsub.on('visibilitychange', function () {
    if (me._state.visible && !me._state.previous.visible) {
      me._pubsub.publish('visible', [me]);
    }
  });

  this._pubsub.on('visibilitychange', function () {
    if (me._state.hidden) {
      me._pubsub.publish('hidden', [me]);
    }
  });
